...
public class BlueMageListener implements Listener {
	public static ConflictClasses plugin;
	public BlueMageListener(ConflictClasses instance) {
		plugin = instance;
	}
	@EventHandler
			...
			for(int i = 0;i<blocks.size();i++){
				...
				for(int j = 0;j<entities.size();j++){
					if(entities.get(j) instanceof LivingEntity && (blocks.get(i).getLocation().equals(((LivingEntity)entities.get(j)).getLocation().getBlock().getLocation())
						...
						try{
							((LivingEntity)entities.get(j)).addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 500, 5));
						}catch(Exception e){}
						blocks.get(i).getWorld().playEffect(blocks.get(i).getLocation(), Effect.EXTINGUISH, null);
					}
				}
			}
			...
			for(int i = 0;i<blocks.size();i++){
				...
				for(int j = 0;j<entities.size();j++){
					if(entities.get(j) instanceof LivingEntity && (blocks.get(i).getLocation().equals(((LivingEntity)entities.get(j)).getLocation().getBlock().getLocation())
							|| blocks.get(i).getLocation().equals(((LivingEntity)entities.get(j)).getEyeLocation().getBlock().getLocation()))){
						try{
							((LivingEntity)entities.get(j)).addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 500, 5));
						}catch(Exception e){}
						blocks.get(i).getWorld().playEffect(blocks.get(i).getLocation(), Effect.EXTINGUISH, null);
					}
				}
