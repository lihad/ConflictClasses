...
public class WhiteMageListener implements Listener {
	public static ConflictClasses plugin;
	public WhiteMageListener(ConflictClasses instance) {
		plugin = instance;
	}
	@EventHandler
			...
			for(int i = 0;i<blocks.size();i++){
				...
				for(int j = 0;j<entities.size();j++){
					if(entities.get(j) instanceof LivingEntity && (blocks.get(i).getLocation().equals(((LivingEntity)entities.get(j)).getLocation().getBlock().getLocation())
						...
						try{
						((LivingEntity)entities.get(j)).setHealth(((LivingEntity)entities.get(j)).getHealth()+1);
						}catch(Exception e){}
						blocks.get(i).getWorld().playEffect(blocks.get(i).getLocation(), Effect.BLAZE_SHOOT, null);
					}
				}
			}
		}
